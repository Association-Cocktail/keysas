...
pub struct VtClient {
    ...
    timeout: Duration,
    /// Host:port used for the internet connectivity probe.
    probe_host: String,
...
impl VtClient {
    ...
    pub fn has_internet(&self) -> bool {
        match self.probe_host.parse() {
            Ok(addr) => TcpStream::connect_timeout(&addr, self.probe_timeout).is_ok(),
            Err(e) => {
                log::warn!("VT probe: invalid probe_host '{}': {e}", self.probe_host);
                false
            }
        }
    }
