<template>
  ...
  <button :class="'btn btn-sm btn-'+ (this.analysingOUT ? 'primary' : 'secondary')" @click="if(this.analysingOUT) {this.analysingOUT = false; this.filesOUT = [{filename:'oof.jpg',is_valid:true,reason:null,detail:null},{filename:'rab.png',is_valid:true,reason:null,detail:null},{filename:'zab.pdf',is_valid:false,reason:'antivirus',detail:'Eicar-Test-Signature'},{filename:'doc.docx',is_valid:false,reason:'specialized',detail:'ALERT: Invoke-Expression (dynamic code execution)'},{filename:'big.iso',is_valid:false,reason:'toobig',detail:null}] } else { this.analysingOUT = true }">analysingOUT</button>
  <button :class="'btn btn-sm btn-'+ (this.filesOUT.length > 0 ? 'primary' : 'secondary')" @click="this.filesOUT = (this.filesOUT.length > 0 ? [] : [{filename:'oof.jpg',is_valid:true,reason:null,detail:null},{filename:'rab.png',is_valid:true,reason:null,detail:null},{filename:'zab.pdf',is_valid:false,reason:'yara',detail:null}])">filesOUT</button>
